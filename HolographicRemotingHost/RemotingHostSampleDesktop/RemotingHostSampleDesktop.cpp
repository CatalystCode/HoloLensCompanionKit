    ...
    if (!m_streamerHelpers)
    {
        ...
        m_streamerHelpers->OnSendFrame += ref new SendFrameEvent(
            [this](_In_ const ComPtr<ID3D11Texture2D>& spTexture, _In_ FrameMetadata metadata)
            {
                ...
                {
                    ComPtr<ID3D11Device1> spDevice = m_appView->GetDeviceResources()->GetD3DDevice();
                    ComPtr<ID3D11DeviceContext> spImmediateContext = m_appView->GetDeviceResources()->GetD3DDeviceContext();
					ComPtr<ID3D11DeviceContext> spContext;
					spDevice->CreateDeferredContext(0, &spContext);
					...
					spContext->FinishCommandList(true, &pCommandList);
					spImmediateContext->ExecuteCommandList(pCommandList, true);
		...
		//m_streamerHelpers->SetEnableAudio(false);
        try
        {
